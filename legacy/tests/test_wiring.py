"""Wiring tests guarding boundaries between our code and PEFT/transformers."""
...
def test_param_groups_classify_and_count():
    ...
    assert len(named["wdown"]) == 2
    assert len(named["new"]) == 2
...
def test_param_groups_reject_unknown_trainable():
    model = FakeModel()
    _freeze_except(model, CFG)
    model.layers[3].mlp.down_proj.weight.requires_grad_(True)
